...
CREATE PROC gnst_SWMESPUDelvInIF_ImpDelv
...
MES_PK_PONo NVARCHAR(20), -- gnst_TMESPUDelvIF
...
CompanySeq , WorkingTag , SETID , MES_PK_PONo , PermitSeq ,
...
'' , C.BizUnit , C.EmpSeq , C.DeptSeq , C.CustSeq ,
...
SELECT A.IDX_NO, A.WorkingTag, A.SETID, A.PermitSeq, A.PermitSeq, A.DelvSeq, A.DelvSerl, 0
...
FROM gnst_TMESPUDelvInIF AS A JOIN \#ImpBL AS B ON A.SETID = B.SETID
...
EXEC \_SWCOMConfirmStopUpdate @CompanySeq, '#Temp_InOutReqItemProg', 'PermitSeq', 'PermitSerl', '', 501077
...
1, 'Qty', 'STDQty', '', '', 'PermitSeq', 'PermitSerl', '', '\_TUIImpPermit', 501077
...
INSERT INTO \#Temp_SourceItem(PermitSeq, PermitSerl, PermitQty, PermitSTDQty, PermitAmt, ProgQty, ProgSTDQty, ProgAmt)
...
AND ((@SMProgCheckKind = 1045001 AND @IsOverFlow \<\> '1' AND C.PermitQty \< C.ProgQty + C.DelvQty ) OR -- 수량

(@SMProgCheckKind = 1045002 AND @IsOverFlow \<\> '1' AND C.PermitSTDQty \< C.ProgSTDQty + C.DelvSTDQty) OR -- 기준단위수량

(@SMProgCheckKind = 1045003 AND @IsOverFlow \<\> '1' AND C.PermitAmt \< C.ProgAmt + C.DelvAmt ) OR -- 금액
...
JOIN \#TUIImpDelv AS Delv ON A.MES_PK_PONo = Delv.MES_PK_PONo
...
--JOIN \#TUIImpDelvItem AS B ON A.POSeq = B.POSeq
...
DELETE gnst_TUIImpDelvItem

FROM gnst_TUIImpDelvItem AS A
...
DELETE gnst_TUIImpDelv

FROM gnst_TUIImpDelv AS A
...
ISNULL(Remark,'') ,@UserSeq ,GETDATE() ,ISNULL(SMImpKind,0) ,ISNULL(IsPJT,'') ,
...
INSERT INTO gnst_TUIImpDelv(CompanySeq, DelvSeq, IsMES, LastUserSeq, LastDateTime)
...
INSERT INTO gnst_TUIImpDelvItem(CompanySeq, DelvSeq, DelvSerl, IsMES, MES_PK_PONo,
...
SET ERPDelvInSeq = B.DelvSeq ,

ERPDelvInSerl = B.DelvSerl,
